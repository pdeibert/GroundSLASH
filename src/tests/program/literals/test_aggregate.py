...
class TestAggregate(unittest.TestCase):
    def test_aggregate_element(self):
        ...
        self.assertEqual(
            element.body, LiteralTuple(PredicateLiteral("p", String("str")), Naf(PredicateLiteral("q", Variable("Y"))))
        )
        ...
        self.assertTrue(element.vars() == element.global_vars() == {Variable("X"), Variable("Y")})
    ...
    def test_aggregate_count(self):
        ...
        # I subset J
        self.assertTrue(
            aggr_func.propagate((Guard(RelOp.GREATER_OR_EQ, Number(1), True), None), element_instances, A, B)
        )
        self.assertTrue(aggr_func.propagate((Guard(RelOp.GREATER, Number(1), True), None), element_instances, A, B))
        # J subset I
        self.assertTrue(
            aggr_func.propagate((Guard(RelOp.GREATER_OR_EQ, Number(1), True), None), element_instances, B, A)
        )
    ...
    def test_aggregate_sum(self):
        ...
        # I subset J
        self.assertTrue(
            aggr_func.propagate((Guard(RelOp.GREATER_OR_EQ, Number(1), True), None), element_instances, I, J)
        )
        self.assertFalse(aggr_func.propagate((Guard(RelOp.GREATER, Number(1), True), None), element_instances, I, J))
        # J subset I
        self.assertFalse(
            aggr_func.propagate((Guard(RelOp.GREATER_OR_EQ, Number(1), True), None), element_instances, J, I)
        )
    ...
    def test_aggregate_literal(self):
        ...
        ground_elements = (
            AggregateElement(
                TermTuple(Number(5)), LiteralTuple(PredicateLiteral("p", String("str")), Naf(PredicateLiteral("q")))
            ),
            AggregateElement(TermTuple(Number(-3)), LiteralTuple(Naf(PredicateLiteral("p", String("str"))))),
        )
        aggr_func = AggregateCount()
        ...
        ground_literal = AggregateLiteral(
            aggr_func, ground_elements, guards=(Guard(RelOp.LESS, Number(3), False), Guard(RelOp.LESS, Number(3), True))
        )
        self.assertEqual(ground_literal.lguard, Guard(RelOp.LESS, Number(3), False))
        self.assertEqual(ground_literal.rguard, Guard(RelOp.LESS, Number(3), True))
        self.assertEqual(
            ground_literal.guards, (Guard(RelOp.LESS, Number(3), False), Guard(RelOp.LESS, Number(3), True))
        )
        self.assertFalse(ground_literal.eval())
        ...
        var_elements = (
            AggregateElement(
                TermTuple(Number(5)), LiteralTuple(PredicateLiteral("p", Variable("X")), Naf(PredicateLiteral("q")))
            ),
            AggregateElement(TermTuple(Number(-3)), LiteralTuple(Naf(PredicateLiteral("p", String("str"))))),
        )
        ...
        self.assertEqual(
            ...
            AggregateLiteral(
                aggr_func,
                ground_elements,
                guards=(Guard(RelOp.LESS, Number(3), False), Guard(RelOp.LESS, Number(3), True)),
            ),
        )
        # hashing
        self.assertEqual(
        ...
        arith_elements = (
            AggregateElement(
                TermTuple(Number(5)),
                LiteralTuple(PredicateLiteral("p", Minus(Variable("X"))), Naf(PredicateLiteral("q"))),
            ),
        )
        ...
        self.assertEqual(
            ...
            AggregateLiteral(
                ...
                (
                    AggregateElement(
                        TermTuple(Number(5)), LiteralTuple(PredicateLiteral("p", Number(1)), Naf(PredicateLiteral("q")))
                    ),
                    AggregateElement(TermTuple(Number(-3)), LiteralTuple(Naf(PredicateLiteral("p", String("str"))))),
                ),
