...
class TestLiteral(unittest.TestCase):
    def test_literal_tuple(self):
        ...
        literals = LiteralTuple(
            PredicateLiteral("p", Number(0), Variable("X")), PredicateLiteral("q", Minus(Variable("Y")))
        )
        # length
        ...
        # match
        self.assertEqual(
            LiteralTuple(PredicateLiteral("p", Variable("X"), String("f")), PredicateLiteral("q", Variable("X"))).match(
                LiteralTuple(PredicateLiteral("p", Number(1), String("f")), PredicateLiteral("q", Number(1)))
            ),
            Substitution({Variable("X"): Number(1)}),
        )
        self.assertEqual(
            LiteralTuple(PredicateLiteral("p", Variable("X"), String("f")), PredicateLiteral("q", Variable("X"))).match(
                LiteralTuple(PredicateLiteral("p", Number(1), String("g")), PredicateLiteral("q", Number(1)))
            ),
            None,
        )  # ground terms don't match
        self.assertEqual(
            LiteralTuple(PredicateLiteral("p", Variable("X"), String("f")), PredicateLiteral("q", Variable("X"))).match(
                LiteralTuple(PredicateLiteral("p", Number(1), String("f")), PredicateLiteral("q", Number(0)))
            ),
            None,
        )  # assignment conflict
        self.assertEqual(
