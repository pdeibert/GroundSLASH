...
from aspy.program.statements import EtaRule, EpsRule, rewrite_aggregate
...
class TestRewrite(unittest.TestCase):
    def test_rewrite_aggregate(self):
        ...
        alpha_literal, eps_rule, eta_rules = rewrite_aggregate(aggregate_1, 1, {Variable('X')}, [PredicateLiteral('q', Variable('X')), Equal(Number(0), Variable('X'))])
        self.assertEqual(alpha_literal, AlphaLiteral(1, TermTuple(Variable('X')), TermTuple(Variable('X'))))
        self.assertEqual(eps_rule,
            EpsRule(
        ...
        alpha_literal, eps_rule, eta_rules = rewrite_aggregate(aggregate_2, 2, {Variable('X')}, [PredicateLiteral('q', Variable('X')), Equal(Number(0), Variable('X'))])
