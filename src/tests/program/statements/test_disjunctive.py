...
class TestDisjunctive:
    def test_disjunctive_fact(self: Self):
        ...
        with pytest.raises(ValueError):
            DisjunctiveRule((PredLiteral("p", Number(0)),))  # not enough atoms
        with pytest.raises(ValueError):
            DisjunctiveRule(
        ...
        assert str(ground_rule) == "p(1) | p(0)."
        assert str(var_rule) == "p(1) | p(X)."
        # equality
        ...
        assert hash(ground_rule) == hash(
            DisjunctiveRule((PredLiteral("p", Number(1)), PredLiteral("p", Number(0))))
        )
        ...
        # TODO:
        # assert ground_rule.safety() == SafetyTriplet())
        # assert var_rule.safety() == SafetyTriplet(unsafe={Variable("X")}))
    ...
    def test_disjunctive_rule(self: Self):
        ...
        # string representation
        assert str(ground_rule) == "p(1) | p(0) :- q."
        assert str(unsafe_var_rule) == "p(1) | p(X) :- q."
        assert str(safe_var_rule) == "p(1) | p(X) :- q(X)."
        # equality
        assert ground_rule.head == LiteralCollection(
            PredLiteral("p", Number(1)), PredLiteral("p", Number(0))
        )
        ...
        assert safe_var_rule.head == LiteralCollection(
            PredLiteral("p", Number(1)), PredLiteral("p", Variable("X"))
        )
