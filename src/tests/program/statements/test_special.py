...
from aspy.program.terms import Number, Variable, TermTuple
from aspy.program.literals import Naf, Neg, PredicateLiteral, LiteralTuple, Guard, GreaterEqual, LessEqual, EpsLiteral, EtaLiteral, AggregateElement
from aspy.program.operators import RelOp
from aspy.program.symbol_table import SpecialChar

from aspy.program.statements import EtaRule, EpsRule
...
class TestSpecial(unittest.TestCase):
    def test_eps_rule(self):
        ...
        ground_rule = EpsRule(ground_eps_literal, *ground_guards, ground_guard_literals + LiteralTuple(PredicateLiteral('p', Number(2), Number(3))))
        ...
        var_rule = EpsRule(var_eps_literal, *var_guards, var_guard_literals + LiteralTuple(PredicateLiteral('p', Number(2), Variable('Y'))))

        # invalid initialization
        self.assertRaises(ValueError, EpsRule.from_scratch, 1, {global_vars}, *ground_guards, base_value, LiteralTuple(PredicateLiteral('p', Number(2), Number(3)))) # non-tuple for 'global_vars'
        self.assertRaises(ValueError, EpsRule.from_scratch, 1, global_vars, Guard(RelOp.GREATER_OR_EQ, Number(-1), False), Guard(RelOp.LESS_OR_EQ, Number(10), False), base_value, LiteralTuple(PredicateLiteral('p', Number(2), Number(3)))) # two left guards
        ...
        self.assertEqual(ground_rule.body[1], LessEqual(Number(0), Number(10))) # NOTE: order of operands in built-in terms
        self.assertEqual(ground_rule.body[2], PredicateLiteral('p', Number(2), Number(3)))
        self.assertEqual(len(var_rule.body), 3)
        self.assertEqual(var_rule.body[0], GreaterEqual(Number(-1), Number(0))) # NOTE: order of operands in built-in terms
        self.assertEqual(var_rule.body[1], LessEqual(Number(0), Variable('X'))) # NOTE: order of operands in built-in terms
        ...
        self.assertEqual(ground_rule.body, ground_guard_literals + LiteralTuple(PredicateLiteral('p', Number(2), Number(3))))
        self.assertEqual(var_rule.head, LiteralTuple(var_eps_literal))
        self.assertEqual(var_rule.body, var_guard_literals + LiteralTuple(PredicateLiteral('p', Number(2), Variable('Y'))))
        # hashing
        self.assertEqual(hash(ground_rule), hash(EpsRule(ground_eps_literal, *ground_guards, ground_guard_literals + LiteralTuple(PredicateLiteral('p', Number(2), Number(3))))))
        self.assertEqual(hash(var_rule), hash(EpsRule(var_eps_literal, *var_guards, var_guard_literals + LiteralTuple(PredicateLiteral('p', Number(2), Variable('Y'))))))
        # ground
    ...
    def test_eta_rule(self):
        ...
        ground_rule = EtaRule(ground_eta_literal, element, LiteralTuple(PredicateLiteral('p', Number(5)), PredicateLiteral('p', Number(2), Number(3))))

        var_eta_literal = EtaLiteral(1, 3, local_vars, global_vars, local_vars+global_vars)
        var_rule = EtaRule(var_eta_literal, element, element.literals + LiteralTuple(PredicateLiteral('p', Number(2), Variable('Y'))))
        ...
        # hashing
        self.assertEqual(hash(ground_rule), hash(EtaRule(ground_eta_literal, element, LiteralTuple(PredicateLiteral('p', Number(5)), PredicateLiteral('p', Number(2), Number(3))))))
        self.assertEqual(hash(var_rule), hash(EtaRule(var_eta_literal, element, element.literals + LiteralTuple(PredicateLiteral('p', Number(2), Variable('Y'))))))
        # ground
        self.assertTrue(ground_rule.ground)
        self.assertFalse(var_rule.ground)
        # safety
