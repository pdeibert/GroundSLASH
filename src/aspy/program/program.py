...
class Program:
    ...
    def reduct(self, preds: Set[Tuple[str, int]]) -> "Program":
        ...
        return Program( tuple(statement for statement in self.statements if not any(literal.pred() in preds for literal in statement.body.neg_occ())) )

    def replace_arith(self) -> "Program":
        return Program(tuple(statement.replace_arith() for statement in self.statements), self.query)

    def rewrite_aggregates(self) -> Tuple["Program", "Program", "Program", Dict[int, Tuple["AlphaLiteral", "EpsRule", List["EtaRule"]]]]:
